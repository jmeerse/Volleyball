...
#write_csv(sets_all, all_box_file #look at github to see what this was.


#### New Stuff for Capstone ####          
#need dates joined to pbp

game_date <- NCAA_game_pbp %>% select(date, game_id)
game_date$game_id <- as.numeric(game_date$game_id)
full_pbp <- left_join(full_pbp, game_date, by = c("ncaa_match_id" = "game_id"))
          
#### trying for set by set points ####
#get rid of lines of NA's 
full_pbp <- full_pbp %>% drop_na(team)
#filter down to relevant info
setpoints <- full_pbp %>% 
  group_by(ncaa_match_id, set_number) %>% 
  summarise(ht = last(home_team),
            home = max(home_score),
            away = max(away_score),
            at = last(away_team),
            id = last(id),
...
#calculate number of sets won, match winner, points and point differential
sets <- setpoints %>% group_by(ncaa_match_id) %>% 
  summarise(ht = last(ht),
            h_sets = sum(home > away),
            at = last(at),
            a_sets = sum(away > home),
            match_id = last(ncaa_match_id),
            h_pts = sum(home),
            a_pts = sum(away),
            date = last(date)
            ) %>% 
  mutate(winner = if_else(h_sets > a_sets, ht, at),
         loser = if_else(h_sets < a_sets, ht, at ),
         pt_diff = if_else(winner == ht, h_pts - a_pts, a_pts - h_pts)
         ) %>% 
  arrange(ncaa_match_id)

sets %>% filter(ht == "Lees") 
full_pbp %>% filter(ncaa_match_id == 5188116)

#need to remove 3 games with Lees_McRae, as their name got assigned to both home and away
#those ncaa_match_id = 5188116  ht = "Queens (NC)" at = "Lees-McRae" winner = "Queens (NC)" loser = "Lees-McRae"
#5215190  ht = "Barton" at = "Lees-McRae" loser = "Barton" winner = "Lees-McRae"
#5221356 ht = "Bluefield Col." at = "Lees-McRae" loser = "Lees-McRae" winner = "Bluefield Col."

#sets <- sets %>% 
#  mutate(ht = ifelse(ncaa_match_id == 5221356,"Bluefield Col.", ht),
#         at = ifelse(ncaa_match_id == 5221356,"Lees-McRae", at),
#        winner = ifelse(ncaa_match_id == 5221356,"Bluefield Col.", winner),
#         loser = ifelse(ncaa_match_id == 5221356,"Lees-McRae", loser)
#           )


win_rec <- sets %>% group_by(winner) %>% summarise(wins = n()) %>% ungroup()
loss_rec <- sets %>% group_by(loser) %>% summarise(losses = n()) %>% ungroup()

records <- left_join(win_rec, loss_rec, by = c("winner" = "loser"))

records <- records %>% mutate(games = wins + losses,
                              pct = wins / games)

records <- na.omit(records)
records <- records %>% filter(games > 10)
records <- records %>% rename(team = winner)
records$pct <- round(records$pct, 3)

sets <- sets %>% mutate(win_pts = ifelse(ht == winner, h_pts, a_pts),
                        lose_pts = ifelse(at == loser,a_pts, h_pts ))

win_pts <- sets %>%
  group_by(team = winner) %>% 
  summarise(pts_for = sum(win_pts),
            pts_against = sum(lose_pts))

lose_pts <- sets %>%
  group_by(team = loser) %>% 
  summarise(pts_for = sum(lose_pts),
            pts_against = sum(win_pts))


win_pts <-  win_pts %>% rename(pts_for_w = pts_for,
                               pts_against_w = pts_against)

lose_pts <-  lose_pts %>% rename(pts_for_l = pts_for,
                               pts_against_l = pts_against)

records <- left_join(records, win_pts, by = "team")
records <- left_join(records, lose_pts, by = "team")

records <- records %>% mutate(pts_for = pts_for_w + pts_for_l,
                              pts_against = pts_against_w + pts_against_l)

p_exp <- 10.8 #number comes from Mathletics
records <- records %>% mutate(pyth_pct = (pts_for ^ p_exp)/((pts_for ^ p_exp)+ (pts_against ^ p_exp)),
                              pyth_wins = round(games * pyth_pct, 2))
...
for (i in exp) {
dat$rmse <- sqrt(mean(
  (records$wins - ((records$pts_for ^ i)/((records$pts_for ^ i)+ (records$pts_against ^ i)) * records$games))^2))
...
min(best_exp$rmse) #so with exp = 8.7, rmse is minimized
#now adjust for best_exp
p_exp <- 8.7
records <- records %>% mutate(pyth_pct = (pts_for ^ p_exp)/((pts_for ^ p_exp)+ (pts_against ^ p_exp)),
                              pyth_wins = round(games * pyth_pct, 2))


records %>% ggplot(aes(x = pyth_pct, y = pct)) + geom_point() + geom_smooth(method = "lm")

cor(x = records$pyth_pct, y = records$pct)
cor(x = records$wins, y = records$pyth_wins)

records %>% ggplot(aes(x = wins, y = pyth_wins)) + 
  geom_point() + 
...
rownames(elo22_20) <- 1:nrow(elo22_20)

elo_all <- left_join(elo22, elo22_20) #if you had run two different k values
...
#ideas: do this for 4-5 years; see how teams change year by year
#extract_elo will calculate for certain dates - see how it changes during a season
#convert from elo to probabilities of winning, predict future games
#run it during this season - see how to update games in downloading results




home_pts <- sets %>% group_by(ht) %>% summarise(home_pts = sum(h_pts),
                                                n = n())

away_pts <- sets %>% group_by(at) %>% summarise(away_pts = sum(a_pts),
                                                n = n())

team_pts <- left_join(home_pts, away_pts, by = c("ht" = "at"))

team_pts %>% filter(n.x + n.y > 15) %>%  
  ggplot(aes(x= home_pts, y = away_pts)) + 
  geom_point() +
  geom_text_repel(aes(label = ht))
